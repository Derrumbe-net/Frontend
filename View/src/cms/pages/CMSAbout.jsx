...
export default function CMSAbout() {
  ...
  const saveFacultyMember = (member) => {
    ...
    if (member.id && data.facultad.find((m) => m.id === member.id)) {
      updatedFacultad = data.facultad.map((m) =>
  ...
  const deleteFaculty = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        setData({
          ...data,
          facultad: data.facultad.filter((m) => m.id !== id),
        });
      }
    });
  };
  ...
  const updateStudentList = (type, newList) => {
    ...
    Swal.fire(
      "Éxito",
      "Lista de estudiantes actualizada en memoria.",
      "success"
    );
  };
  ...
  return (
    <div className="cms-card" style={{ padding: "20px" }}>
      ...
      <div style={{ display: "flex", gap: "10px", margin: "15px 0" }}>
        ...
        />
        <button className="cms-btn" onClick={addItem}>
          <FaPlus />
        </button>
      </div>
